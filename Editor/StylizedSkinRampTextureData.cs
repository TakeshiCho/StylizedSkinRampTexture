...
{
    public class StylizedSkinRampTextureData : ScriptableObject
    {
        // Serialized Field
        ...
        private static StylizedSkinRampTextureData _default;

        public static StylizedSkinRampTextureData InitNewData(Gradient surface, Gradient subSurface, int size)
        {
            var data = ScriptableObject.CreateInstance<StylizedSkinRampTextureData>();
        ...
        public static StylizedSkinRampTextureData Default()
        {
            if (!_default)
            {
                _default = InitNewData(
                    new Gradient()
                    {
                        colorKeys = new[]
                        {
                    ...
                    128);
                _default.subSurface.mode = GradientMode.Blend;
                _default.surface.mode = GradientMode.Blend;
        ...
        public static void Save(string assetPath,StylizedSkinRampTextureData data)
        {
        ...
        static bool TryFromJson( string json, out StylizedSkinRampTextureData data)
        {
            ...
            try
            {
                data = ScriptableObject.CreateInstance<StylizedSkinRampTextureData>();
                JsonUtility.FromJsonOverwrite(json,data);
                got = true;
            }
            catch(Exception e)
            {
                data = StylizedSkinRampTextureData.Default();
            ...
            finally
            {
                ...
                {
                    var defaultData = StylizedSkinRampTextureData.Default();
                    data ??= defaultData;
